  ...
  <article class="halfContent" id="mainInformation">
    ...
    <div id="locationsBlock" class="clearfix">
        ...
        [#if activity.locations?has_content]
          ...
          <div id="location-head" class="row clearfix">
              ...
              <div id="locationLevel-head" class="locationLevel grid_2 ">
                [#-- Type/Level --]
                <strong>[@s.text name="planning.locations.level" /]</strong>
              </div>
              <div id="locationLatitude-head" class="locationLatitude grid_2">
                [#-- Latitude --]
                <strong>[@s.text name="planning.locations.latitude" /]</strong>
              </div>
              <div id="locationLongitude-head" class="locationLatitude grid_2">
                [#-- Longitude --]
                <strong>[@s.text name="planning.locations.longitude" /]</strong>
              </div>
              <div id="locationName-head" class="locationName grid_3">
                [#-- Name --]
                <strong>[@s.text name="planning.locations.name" /]</strong>
              </div>
            </div> 
          [#list activity.locations as location]
            ...
            <div id="location-${location_index}" class="location row borderBox clearfix">
              ...
              <div class="locationLevel grid_2 ">
                [#-- Type/Level --]
                [#if location.region]
                  [@customForm.select name="activity.locations[${location_index}].type.id" i18nkey="planning.locations.level" listName="locationTypes" keyFieldName="id"  displayFieldName="name" showTitle=false value="${regionTypeID}"/]
                [#elseif location.country]
                  [@customForm.select name="activity.locations[${location_index}].type.id" i18nkey="planning.locations.level" listName="locationTypes" keyFieldName="id"  displayFieldName="name" showTitle=false value="${countryTypeID}"/]
                [#else]
                  [@customForm.select name="activity.locations[${location_index}].type.id" i18nkey="planning.locations.level" listName="locationTypes" keyFieldName="id"  displayFieldName="name" showTitle=false value="${location.type.id}"/]
                [/#if]
              </div>
              <div class="locationLatitude grid_2">
                [#-- Latitude --]
                [#if location.country || location.region]
                  [@customForm.input name="geoPosition.latitude" type="text" value=notApplicableText i18nkey="planning.locations.latitude" showTitle=false disabled=true  /]
                [#else]
                  [@customForm.input name="activity.locations[${location_index}].geoPosition.latitude" type="text" i18nkey="planning.locations.latitude" showTitle=false required=true  /]
                [/#if]
              </div>
              <div class="locationLongitude grid_2">
                ...
                [#else]
                  [@customForm.input name="activity.locations[${location_index}].geoPosition.longitude" type="text" i18nkey="planning.locations.longitude" showTitle=false required=true  /]
                [/#if]
              </div>
              <div class="locationName grid_3">
                [#-- Name --]
                [#if location.country]
                  [@customForm.select name="activity.locations[${location_index}].id" i18nkey="planning.locations.level" listName="countries" keyFieldName="id" showTitle=false  displayFieldName="name" value="${location.id?int}" /]
                [#elseif location.region]
                  [@customForm.select name="activity.locations[${location_index}].id" i18nkey="planning.locations.level" listName="regions" keyFieldName="id" showTitle=false  displayFieldName="name" value="${location.id?int}" /]
                [#else]
                  <input type="hidden" name="activity.locations[${location_index}].id" value="${location.id?int}">
                  [@customForm.input name="activity.locations[${location_index}].name" type="text" i18nkey="planning.locations.notApplicable" required=true showTitle=false  /]
                [/#if]
              </div>
      ...
      <div id="addLocationBlock" class="addLink">
        ...
        <a href="" id="addLocationLink" class="addLocation" >[@s.text name="planning.location.addLocation" /]</a>
      </div>  
    </div> 
