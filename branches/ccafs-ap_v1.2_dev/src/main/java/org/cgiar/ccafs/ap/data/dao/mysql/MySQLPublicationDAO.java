...
public class MySQLPublicationDAO implements PublicationDAO {
  ...
  public List<Map<String, String>> getPublications(int leaderId, int logframeId) {
    ...
    try (Connection connection = dbManager.getConnection()) {
      String query =
        "SELECT p.id, p.identifier, p.citation, p.file_url, p.isi_publication, p.nars_coauthor, p.earth_system_coauthor, "
          + "pt.id as 'publication_type_id', pt.name as 'publication_type_name', oa.id as 'publication_access_id', "
    ...
    try (Connection connection = dbManager.getConnection()) {
      ...
      addQueryPrepared =
        "INSERT INTO publications (id, publication_type_id, identifier, citation, file_url, logframe_id, "
          + "activity_leader_id, open_access_id, isi_publication, nars_coauthor, earth_system_coauthor) "
          + "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
      ...
      values[8] = publication.get("isi_publication");
      values[9] = publication.get("nars_coauthor");
      values[10] = publication.get("earth_system_coauthor");
