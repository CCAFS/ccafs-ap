...
public class MySQLNextUserDAO implements NextUserDAO {
  ...
  public int saveNextUser(int deliverableID, Map<String, Object> nextUserData) {
    ...
    if (nextUserData.get("id") == null) {
      ...
      query.append("INSERT INTO next_users (deliverable_id, user, expected_changes, strategies) ");
      query.append("VALUES (?,?,?,?) ");
      values = new Object[4];
      ...
      result = databaseManager.saveData(query.toString(), values);
      if (result == -1) {
        LOG.error("A problem happened trying to update the next user record", nextUserData.get("id"));
        return -1;
      }
    }
