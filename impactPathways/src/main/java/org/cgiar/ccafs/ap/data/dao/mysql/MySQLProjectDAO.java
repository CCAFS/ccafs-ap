...
public class MySQLProjectDAO implements ProjectDAO {
  ...
  public int saveProject(Map<String, Object> projectData) {
    ...
    if (projectData.get("id") == null) {
      ...
      query.append(
        "bilateral_contract_name = ?, modified_by = ?, modification_justification = ?, annual_report_to_dornor=? ");
      query.append("WHERE id = ?");
  ...
  public List<Map<String, Object>> summaryGetActiveProjects() {
    ...
    query.append(
      "( SELECT GROUP_CONCAT(DISTINCT CONCAT( i.acronym, '@', i.name, IFNULL(CONCAT('@', i.website_link) ,' ')) SEPARATOR ';' ) FROM project_partners pp INNER JOIN institutions i ON pp.institution_id = i.id WHERE p.id = pp.project_id AND pp.is_active = 1 GROUP BY p.id) as 'partners', ");
    query.append(
      "( SELECT GROUP_CONCAT(DISTINCT CONCAT( pro.acronym, '@', pro.name ) SEPARATOR ';') FROM ip_programs pro INNER JOIN project_focuses pf ON pro.id = pf.program_id WHERE pf.project_id = p.id AND pro.region_id IS NULL ) as 'flagships', ");
    query.append(
      "( SELECT GROUP_CONCAT(DISTINCT CONCAT( pro.acronym, '@', pro.name ) SEPARATOR ';') FROM ip_programs pro INNER JOIN project_focuses pf ON pro.id = pf.program_id WHERE pf.project_id = p.id AND pro.region_id IS NOT NULL ) as 'regions', ");
    query.append(
      "( SELECT GROUP_CONCAT(DISTINCT CONCAT( le.id, CONCAT('@', let.name), CONCAT('@', le.name), IFNULL(CONCAT('@', lg.latitude) ,' ') , IFNULL(CONCAT('@', lg.longitude),' '), IFNULL(CONCAT('@', le.code) ,' ')) SEPARATOR ';') ");
    query.append(
      "FROM project_locations po INNER JOIN loc_elements le ON po.loc_element_id = le.id INNER JOIN loc_element_types let ON le.element_type_id = let.id ");
    ...
    query.append(
      "( SELECT GROUP_CONCAT( DISTINCT CONCAT(pj.id, '@@', pj.title) SEPARATOR ';' ) FROM project_cofinancing_linkages pcl INNER JOIN projects pj ON pcl.bilateral_project_id = pj.id WHERE p.type = 'CCAFS_COFUNDED' AND p.id = pcl.core_project_id AND pcl.is_active = 1 GROUP BY p.id) as 'cofunded_contributions', ");
    query.append(
      "( SELECT GROUP_CONCAT( DISTINCT CONCAT(pj.id, '@@', pj.title) SEPARATOR ';' ) FROM project_cofinancing_linkages pcl INNER JOIN projects pj ON pcl.core_project_id = pj.id WHERE p.type = 'BILATERAL' AND p.id = pcl.bilateral_project_id AND pcl.is_active = 1 GROUP BY p.id) as 'bilateral_contributions', ");
  ...
  public List<Map<String, Object>> summaryGetAllCCAFSOutcomes(int year) {
    ...
    query.append(" where ie.id in (ipe.id)) end AS  'flagship' ");
