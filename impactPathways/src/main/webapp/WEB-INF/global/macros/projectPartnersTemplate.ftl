...
[#macro partner ap ap_index ap_name  editable=false isPPA=false canEdit=true  responsabilities=false   ]
  <div id="projectPartner-${ap_index}" class="projectPartner borderBox">
    ...
    <div class="fullPartBlock partnerName chosen">
      [@customForm.select name="${ap_name}[${ap_index}].partner" label=""  disabled=!editable i18nkey="preplanning.projectPartners.partner.name" listName="allPartners" keyFieldName="id"  displayFieldName="getComposedName()" value="${ap.institution.id?c}" /]
    </div>
    [#-- Filters --]
    [#if editable]
      [#if canEdit && ap.id != -1]
