    ...
    <li class="">
      ...
      <ul>
        [#list 1..5 as indicator]  
          [@menu actionName="crpIndicators" stageName="indicator-${indicator_index+1}" textName="Indicator type ${indicator_index+1}" /]
        [/#list]
      </ul>
    </li>
  </ul>
    ...
    <li id="menu-${actionName}" class="[#if currentSubStage == stageName]${currCss}[/#if]">
      ...
      [#else]
        [#if canEdit && !sectionCompleted(actionName)]
          <a href="[@s.url action=actionName][@s.param name='liaisonInstitutionID']${(liaisonInstitutionID)!}[/@s.param][@s.param name='indicatorTypeID']${(indicatorTypeID)!}[/@s.param][@s.param name='edit']true[/@s.param][/@s.url]" title="[#if subText != ""]${subText}[/#if]">
            [@s.text name=textName /] 
          </a> 
        [#else]
          <a href="[@s.url action=actionName][@s.param name='liaisonInstitutionID']${(liaisonInstitutionID)!}[/@s.param][@s.param name='indicatorTypeID']${(indicatorTypeID)!}[/@s.param][/@s.url]" title="[#if subText != ""]${subText}[/#if]">
            [@s.text name=textName /]
          </a> 
        [/#if]
      [/#if]
