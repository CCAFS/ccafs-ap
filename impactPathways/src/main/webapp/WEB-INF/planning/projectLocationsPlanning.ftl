              ...
              [#list project.locations as location]
                ...
                <div id="location-${location_index}" class="location row clearfix">
                  ...
                  <div class="locationName grid_3">
                    [#if location.country]
                      [@customForm.select name="project.locations" i18nkey="planning.project.locations.level" listName="countries" keyFieldName="id" showTitle=false  displayFieldName="name" value="${location.id}" disabled=!editable/]
                    [#elseif location.region]
                      [@customForm.select name="project.locations" i18nkey="planning.project.locations.level" listName="regions" keyFieldName="id" showTitle=false  displayFieldName="name" value="${location.id}" disabled=!editable/]
                    [#elseif location.climateSmartVillage]
                      [@customForm.select name="project.locations" i18nkey="planning.project.locations.level" listName="climateSmartVillages" keyFieldName="id" showTitle=false  displayFieldName="name" value="${location.id}" disabled=!editable/]
                    [#else]
                     [#if location.type.id == ccafsSiteTypeID] 
