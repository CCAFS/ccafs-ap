...
<section class="content">
  ...
  [#include "/WEB-INF/planning/planningDataSheet.ftl" /]
    ...
    [#if !saveable]
      <p class="readPrivileges">
        [@s.text name="saving.read.privileges"]
          [@s.param][@s.text name="planning.project.locations.title"/][/@s.param]
        [/@s.text]
      </p>
    [/#if]
    [#-- Title --]
    ...
    <div id="locationsGlobalBlock" class="clearfix">
      [@customForm.checkbox className="globalCheck" name="activity.global" i18nkey="planning.project.locations.checkbox.isGlobal" checked=true value="true" /]
    ...
    <div id="locationsBlock" class="clearfix">
      <div id="fields">
        [#if activity?has_content]
          ...
          <div id="location-head" class="row clearfix">
            ...
            <div id="locationLevel-head" class="locationLevel grid_2">
              <strong>[@s.text name="planning.project.locations.level" /]</strong>
            </div>
            [#-- Latitude --]
            <div id="locationLatitude-head" class="locationLatitude grid_2">
              <strong>[@s.text name="planning.project.locations.latitude" /]</strong>
            </div>
            [#-- Longitude --]
            <div id="locationLongitude-head" class="locationLatitude grid_2">
              <strong>[@s.text name="planning.project.locations.longitude" /]</strong>
            </div>
            [#-- Name --]
            <div id="locationName-head" class="locationName grid_3">
              <strong>[@s.text name="planning.project.locations.name" /]</strong>
            </div>
          </div> 
        ...
        [#else]
          ...
          [#if !saveable]
            <p>[@s.text name="planning.project.locations.empty" /]</p>
          [/#if]
        [/#if]
    ...
    [#if saveable]
      ...
      <!-- internal parameter -->
      [#if activity?has_content]
        [#if activity.global]<input type="hidden" id="isGlobal" value="${activity.global?string}">[/#if]
