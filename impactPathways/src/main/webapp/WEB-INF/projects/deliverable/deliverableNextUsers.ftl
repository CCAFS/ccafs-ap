  ...
  <div class="fullBlock">
    ...
    [#if deliverable.nextUsers?has_content]
      [#list deliverable.nextUsers as nu] 
        [@deliverableTemplate.nextUserTemplate nu_name=params.nextUsers.name nu_index="${nu_index}" nextUserValue="${nu.id}" editable=editable && action.hasProjectPermission("main", project.id) canEdit=canEdit /]
      [/#list]
    [#else]
    ...
    [/#if]
    [#if editable && action.hasProjectPermission("main", project.id)]
