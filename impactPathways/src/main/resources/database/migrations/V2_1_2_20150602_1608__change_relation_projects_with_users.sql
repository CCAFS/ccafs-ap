...
BEGIN
  ...
  IF EXISTS ((SELECT * FROM information_schema.columns WHERE table_schema=DATABASE() AND table_name='projects' AND column_name='liaison_user_id')) THEN
    ...
    ALTER TABLE projects ADD CONSTRAINT `FK_projects__liaison_users` FOREIGN KEY (`liaison_user_id`) REFERENCES `liaison_users` (`id`) ON DELETE CASCADE ON UPDATE CASCADE;
