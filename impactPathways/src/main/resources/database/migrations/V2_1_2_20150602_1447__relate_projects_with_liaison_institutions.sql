...
BEGIN
  ...
  IF NOT EXISTS ((SELECT * FROM information_schema.columns WHERE table_schema=DATABASE() AND table_name='projects' AND column_name='liaison_institution_id')) THEN
    ALTER TABLE projects ADD `liaison_institution_id` bigint(20) DEFAULT NULL COMMENT 'foreign key to the table liaison institutions' AFTER `program_creator_id`;
    ...
    UPDATE projects p, 
      (
        SELECT id, program_creator_id, 
        CASE program_creator_id 
        WHEN 5 THEN 7 
        WHEN 6 THEN program_creator_id
        WHEN 7 THEN 10
        WHEN 8 THEN program_creator_id
        WHEN 9 THEN program_creator_id
        ELSE program_creator_id +1 END as liaison_institution
        FROM projects 
      ) t 
  ...
  IF EXISTS ((SELECT * FROM information_schema.columns WHERE table_schema=DATABASE() AND table_name='projects' AND column_name='program_creator_id')) THEN
    ALTER TABLE `projects` 
    DROP FOREIGN KEY `FK_projects_program_id`;
    ALTER TABLE `projects` 
    DROP COLUMN `program_creator_id`,
